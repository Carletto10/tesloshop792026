...
@Component({
  ...
  styleUrl: './route-test.component.css'
})
export class RouteTestComponent {
  expectedOrder = ['this.', 'getDataFromApi()', '.subscribe()', 'next :', '(data)', ' { this.usuarios = data;', 'console.log }'];
  codeExample = 'this.getDataFromApi().subscribe({ next: (data) => { this.usuarios = data; console.log(data) } })';
